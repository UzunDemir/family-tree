 ...
 * + Бесконечные корни: процедурная генерация предков
 */
...
const CONFIG = {
  ...
  infiniteRoots: {
    enabled: true,
    loadDepth: 3,          // на каком поколении начинать автозагрузку
    autoLoadDelay: 800,    // мс после зума, прежде чем догрузить
    maxGenerations: 12,    // защита от бесконечной памяти
    namesMale: [
...
const zoom = d3.zoom()
  ...
  .on('zoom', (event) => {
    ...
    // Триггер автозагрузки при сильном приближении
...
function applyJitter(node) {
  ...
  // Только для узлов, у которых ещё нет jitter
...
function getGenerationStyle(generation) {
  // После maxGeneration — продолжаем уменьшать
...
function renderTree(animateEntrance = false) {
  ...
  // === Пульсы ===
  renderPulses(animateEntrance);

  // === Статистика ===
  updateStats();
}

function renderPulses(animateEntrance = false) {
  ...
  pulses = pulsesEnter.merge(pulses);
}
...
function generateAncestor(childId, childBirth, childGeneration, side) {
  ...
  // Дата рождения: на 25-35 лет раньше ребёнка
  let birth = null;
  if (childBirth) {
    ...
    if (yearMatch) {
      ...
      birth = `${String(Math.floor(Math.abs(hashNoise(id, 6)) * 28) + 1).padStart(2, '0')}.${String(Math.floor(Math.abs(hashNoise(id, 7)) * 12) + 1).padStart(2, '0')}.${Math.floor(parentYear)}`;
    }
  }
...
// Загружает следующее поколение для указанного узла
function loadNextGeneration(datum) {
  ...
  const childId = datum.data.id;
  const childBirth = datum.data.birth;
  const childGen = datum.depth;

  // Генерируем отца и мать
  const father = generateAncestor(childId, childBirth, childGen, 'father');
  const mother = generateAncestor(childId, childBirth, childGen, 'mother');

  // Строим поддерево для D3
  const newChildren = [father, mother].map(p => {
    const newDatum = {
      id: p.id,
      name: p.name,
      birth: p.birth,
  ...
  // Добавляем как children
  datum.data.children = datum.data.children || [];
  newChildren.forEach(c => datum.data.children.push(c));
...
// Автозагрузка при приближении
let autoLoadTimer = null;
function scheduleAutoLoad() {
  clearTimeout(autoLoadTimer);
  autoLoadTimer = setTimeout(() => {
    // Находим самые глубокие узлы на экране
    const deepest = root.descendants()
      .filter(d => d.depth >= IR.loadDepth)
      .sort((a, b) => b.depth - a.depth);

    if (deepest.length === 0) return;

    // Грузим у топ-3 самых глубоких
    let loaded = 0;
    ...
    if (loaded > 0) {
      // Пересчитываем layout и перерисовываем
      treeLayout(root);
      root.descendants().forEach(applyJitter);
      renderTree(true);

      // Обновляем статистику
      const deepestNow = root.descendants().reduce((max, d) => Math.max(max, d.depth), 0);
      const status = document.getElementById('infinite-status');
      if (status) {
        status.textContent = `🌌 Глубина рода: ${deepestNow} поколений`;
...
// Ручная загрузка по клику на глубокий узел
...
function attachNodeHandlers(selection) {
  ...
  selection.on('click', (event, d) => {
    ...
    // Если это глубокий узел — догружаем предков
...
// === ИНДИКАТОР ГЛУБИНЫ ===
// Добавляем в футер элемент для отображения глубины
